...
def _setup_logging(verbose: int, run_dir: str) -> logging.Logger:
    """
    Configure un logger root selon le niveau verbose.

    verbose=0 : aucun fichier log — les messages INFO ne sortent que dans le terminal
                via tqdm.write (géré manuellement dans le code).
    verbose>=1 : tous les messages INFO+ vont dans run_dir/run.log ET dans le terminal.
    """
...
def vprint(msg: str, verbose: int, min_level: int = 1, logger: logging.Logger = None):
    """
    Affiche msg si verbose >= min_level.
    Si un logger est fourni, écrit aussi dans le fichier log (verbose >= 1).
    Pour verbose=0, les messages importants sont écrits via tqdm.write.
    """
...
def main():
    ...
    parser.add_argument('--verbose', type=int, choices=[0, 1, 2], default=0,
                        help=(
                            ...
                            "  0 : JSON résultat + tqdm + résumé final (terminal uniquement)\n"
                            "  1 : niveau 0 + fichier log complet (run.log)\n"
    ...
    if args.agent == 'optimal':
        ...
        vprint(f"Starting Java server (mode={java_mode})...", args.verbose, min_level=1, logger=logger)
...
def _run_q_learning_session(env, train_fn, args, max_steps_config, log_file,
    ...
    # Plots — toujours générés, dans run_dir/
    ...
    # Résumé final (toujours affiché dans le terminal)
